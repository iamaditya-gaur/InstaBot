...
# Method to retrieve the recent posts , let's you choose b/w your posts or using different User ID
def get_posts():
    ans = raw_input("1. Fetch your own Media posts.\n2. Fetch someone else's Media posts.")
    if ans == '1':
        user_info = BASE_URL + ("users / self / media / recent /?access_token =%s") %(APP_ACCESS_TOKEN).json()
        if user_info["meta"]["code"] is 200:
            if len(user_info["data"]) :
